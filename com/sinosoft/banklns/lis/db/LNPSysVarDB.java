...
/*     */ 
/*     */ public class LNPSysVarDB extends LNPSysVarSchema
/*     */ {
...
/*     */ 
/*     */   public LNPSysVarDB(Connection tConnection)
/*     */   {
/*  32 */     this.con = tConnection;
/*  33 */     this.db = new DBOper(this.con, "LNPSysVar");
/*  34 */     this.mflag = true;
/*     */   }
/*     */ 
/*     */   public LNPSysVarDB()
/*     */   {
/*  39 */     this.con = null;
/*  40 */     this.db = new DBOper("LNPSysVar");
/*  41 */     this.mflag = false;
/*     */   }
/*     */ 
/*     */   public boolean deleteSQL()
/*     */   {
/*  47 */     LNPSysVarSchema tSchema = getSchema();
...
/*  57 */     tError.moduleName = "LNPSysVarDB";
...
/*     */   public int getCount()
/*     */   {
/*  67 */     LNPSysVarSchema tSchema = getSchema();
/*     */ 
...
/*  75 */       tError.moduleName = "LNPSysVarDB";
...
/*     */     try
/*     */     {
/*  96 */       pstmt = this.con.prepareStatement("DELETE FROM LNPSysVar WHERE  SysVar = ?");
/*  97 */       if ((getSysVar() == null) || (getSysVar().equals("null")))
/*  98 */         pstmt.setNull(1, 12);
/*     */       else {
/* 100 */         pstmt.setString(1, getSysVar());
/*     */       }
...
/* 108 */       tError.moduleName = "LNPSysVarDB";
...
/*     */     try
/*     */     {
/* 145 */       pstmt = this.con.prepareStatement("UPDATE LNPSysVar SET  SysVar = ? , SysVarType = ? , SysVarValue = ? WHERE  SysVar = ?");
/* 146 */       if ((getSysVar() == null) || (getSysVar().equals("null")))
/* 147 */         pstmt.setNull(1, 12);
/*     */       else {
/* 149 */         pstmt.setString(1, getSysVar());
/*     */       }
/* 151 */       if ((getSysVarType() == null) || (getSysVarType().equals("null")))
/* 152 */         pstmt.setNull(2, 12);
/*     */       else {
/* 154 */         pstmt.setString(2, getSysVarType());
/*     */       }
/* 156 */       if ((getSysVarValue() == null) || (getSysVarValue().equals("null")))
/* 157 */         pstmt.setNull(3, 12);
/*     */       else {
/* 159 */         pstmt.setString(3, getSysVarValue());
/*     */       }
/*     */ 
/* 162 */       if ((getSysVar() == null) || (getSysVar().equals("null")))
/* 163 */         pstmt.setNull(4, 12);
/*     */       else {
/* 165 */         pstmt.setString(4, getSysVar());
/*     */       }
...
/* 173 */       tError.moduleName = "LNPSysVarDB";
...
/*     */     try
/*     */     {
/* 210 */       pstmt = this.con.prepareStatement("INSERT INTO LNPSysVar VALUES( ? , ? , ?)");
/* 211 */       if ((getSysVar() == null) || (getSysVar().equals("null")))
/* 212 */         pstmt.setNull(1, 12);
/*     */       else {
/* 214 */         pstmt.setString(1, getSysVar());
/*     */       }
/* 216 */       if ((getSysVarType() == null) || (getSysVarType().equals("null")))
/* 217 */         pstmt.setNull(2, 12);
/*     */       else {
/* 219 */         pstmt.setString(2, getSysVarType());
/*     */       }
/* 221 */       if ((getSysVarValue() == null) || (getSysVarValue().equals("null")))
/* 222 */         pstmt.setNull(3, 12);
/*     */       else {
/* 224 */         pstmt.setString(3, getSysVarValue());
/*     */       }
/*     */ 
...
/* 233 */       tError.moduleName = "LNPSysVarDB";
...
/*     */     try
/*     */     {
/* 271 */       pstmt = this.con.prepareStatement("SELECT * FROM LNPSysVar WHERE  SysVar = ?", 
/* 272 */         1003, 1007);
/* 273 */       if ((getSysVar() == null) || (getSysVar().equals("null")))
/* 274 */         pstmt.setNull(1, 12);
/*     */       else {
/* 276 */         pstmt.setString(1, getSysVar());
/*     */       }
...
/* 287 */           tError.moduleName = "LNPSysVarDB";
...
/* 327 */       tError.moduleName = "LNPSysVarDB";
...
/*     */   }
/*     */ 
/*     */   public LNPSysVarSet query()
/*     */   {
/* 360 */     Statement stmt = null;
/* 361 */     ResultSet rs = null;
/* 362 */     LNPSysVarSet aLNPSysVarSet = new LNPSysVarSet();
/*     */ 
/* 364 */     if (!this.mflag) {
...
/* 371 */       SQLString sqlObj = new SQLString("LNPSysVar");
/* 372 */       LNPSysVarSchema aSchema = getSchema();
...
/*     */       {
/* 380 */         i++;
/* 381 */         LNPSysVarSchema s1 = new LNPSysVarSchema();
/* 382 */         s1.setSchema(rs, i);
/* 383 */         aLNPSysVarSet.add(s1);
/*     */       }try {
...
/* 392 */       tError.moduleName = "LNPSysVarDB";
...
/*     */       }
/*     */     }
/* 419 */     return aLNPSysVarSet;
/*     */   }
/*     */ 
/*     */   public LNPSysVarSet executeQuery(String sql)
/*     */   {
/* 424 */     Statement stmt = null;
/* 425 */     ResultSet rs = null;
/* 426 */     LNPSysVarSet aLNPSysVarSet = new LNPSysVarSet();
/*     */ 
/* 428 */     if (!this.mflag) {
...
/*     */       {
/* 440 */         i++;
/* 441 */         LNPSysVarSchema s1 = new LNPSysVarSchema();
/* 442 */         if (!s1.setSchema(rs, i))
/*     */         {
/* 445 */           CError tError = new CError();
/* 446 */           tError.moduleName = "LNPSysVarDB";
...
/* 460 */       tError.moduleName = "LNPSysVarDB";
...
/*     */       }
/*     */     }
/* 487 */     return aLNPSysVarSet;
/*     */   }
/*     */ 
/*     */   public LNPSysVarSet query(int nStart, int nCount)
/*     */   {
/* 492 */     Statement stmt = null;
/* 493 */     ResultSet rs = null;
/* 494 */     LNPSysVarSet aLNPSysVarSet = new LNPSysVarSet();
/*     */ 
/* 496 */     if (!this.mflag) {
...
/* 503 */       SQLString sqlObj = new SQLString("LNPSysVar");
/* 504 */       LNPSysVarSchema aSchema = getSchema();
...
/*     */           break;
/*     */         }
/* 522 */         LNPSysVarSchema s1 = new LNPSysVarSchema();
/* 523 */         s1.setSchema(rs, i);
/* 524 */         aLNPSysVarSet.add(s1);
/*     */       }try {
...
/* 533 */       tError.moduleName = "LNPSysVarDB";
...
/*     */       }
/*     */     }
/* 560 */     return aLNPSysVarSet;
/*     */   }
/*     */ 
/*     */   public LNPSysVarSet executeQuery(String sql, int nStart, int nCount)
/*     */   {
/* 565 */     Statement stmt = null;
/* 566 */     ResultSet rs = null;
/* 567 */     LNPSysVarSet aLNPSysVarSet = new LNPSysVarSet();
/*     */ 
/* 569 */     if (!this.mflag) {
...
/*     */           break;
/*     */         }
/* 591 */         LNPSysVarSchema s1 = new LNPSysVarSchema();
/* 592 */         if (!s1.setSchema(rs, i))
/*     */         {
/* 595 */           CError tError = new CError();
/* 596 */           tError.moduleName = "LNPSysVarDB";
...
/* 610 */       tError.moduleName = "LNPSysVarDB";
...
/* 651 */       SQLString sqlObj = new SQLString("LNPSysVar");
/* 652 */       LNPSysVarSchema aSchema = getSchema();
/* 653 */       sqlObj.setSQL(2, aSchema);
/* 654 */       String sql = "update LNPSysVar " + sqlObj.getUpdPart() + " where " + strWherePart;
/*     */ 
/* 656 */       int operCount = stmt.executeUpdate(sql);
/* 657 */       if (operCount == 0)
/*     */       {
/* 660 */         CError tError = new CError();
/* 661 */         tError.moduleName = "LNPSysVarDB";
...
/* 681 */       tError.moduleName = "LNPSysVarDB";
...
/* 718 */       tError.moduleName = "LNPSysVarDB";
...
/* 738 */       tError.moduleName = "LNPSysVarDB";
...
/* 785 */       tError.moduleName = "LNPSysVarDB";
...
/* 798 */       tError.moduleName = "LNPSysVarDB";
...
/*     */   }
/*     */ 
/*     */   public LNPSysVarSet getData()
/*     */   {
/* 832 */     int tCount = 0;
/* 833 */     LNPSysVarSet tLNPSysVarSet = new LNPSysVarSet();
/* 834 */     LNPSysVarSchema tLNPSysVarSchema = null;
/* 835 */     if (this.mResultSet == null)
/*     */     {
/* 837 */       CError tError = new CError();
/* 838 */       tError.moduleName = "LNPSysVarDB";
...
/*     */     {
/* 846 */       tCount = 1;
/* 847 */       tLNPSysVarSchema = new LNPSysVarSchema();
/* 848 */       tLNPSysVarSchema.setSchema(this.mResultSet, 1);
/* 849 */       tLNPSysVarSet.add(tLNPSysVarSchema);
/*     */ 
/* 851 */       while (tCount++ < 5000)
/*     */       {
/* 853 */         if (!this.mResultSet.next())
/*     */           continue;
/* 855 */         tLNPSysVarSchema = new LNPSysVarSchema();
/* 856 */         tLNPSysVarSchema.setSchema(this.mResultSet, 1);
/* 857 */         tLNPSysVarSet.add(tLNPSysVarSchema);
/*     */       }
/*     */ 
/*     */     }
/*     */     catch (Exception ex)
...
/* 864 */       tError.moduleName = "LNPSysVarDB";
...
/* 904 */         tError.moduleName = "LNPSysVarDB";
...
/* 919 */       tError.moduleName = "LNPSysVarDB";
...
/* 930 */         tError.moduleName = "LNPSysVarDB";
...
/* 945 */       tError.moduleName = "LNPSysVarDB";
 ...
 * Qualified Name:     com.sinosoft.banklns.lis.db.LNPSysVarDB
