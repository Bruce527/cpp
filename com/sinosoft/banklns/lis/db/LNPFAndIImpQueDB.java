...
/*   33 */     this.db = new DBOper(this.con, "LNPFAndIImpQue");
/*   34 */     this.mflag = true;
/*      */   }
/*      */ 
...
/*      */   {
/*   39 */     this.con = null;
/*   40 */     this.db = new DBOper("LNPFAndIImpQue");
/*   41 */     this.mflag = false;
/*      */   }
/*      */ 
/*      */   public boolean deleteSQL()
/*      */   {
/*   47 */     LNPFAndIImpQueSchema tSchema = getSchema();
...
/*   57 */     tError.moduleName = "LNPFAndIImpQueDB";
...
/*      */   public int getCount()
/*      */   {
/*   67 */     LNPFAndIImpQueSchema tSchema = getSchema();
/*      */ 
...
/*   75 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*      */     try
/*      */     {
/*   96 */       pstmt = this.con.prepareStatement("DELETE FROM LNPFAndIImpQue WHERE  Questionid = ? AND InnerOrder = ?");
...
/*  113 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*      */     try
/*      */     {
/*  150 */       pstmt = this.con.prepareStatement("UPDATE LNPFAndIImpQue SET  ImpartId = ? , Questionid = ? , QuestionType = ? , Content1 = ? , Content2 = ? , QueOrder = ? , InnerOrder = ? , OutQueId = ? , InnerQueNum = ? , ParentId = ? , ChildQueNum = ? , Operator = ? , ManageCom = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? WHERE  Questionid = ? AND InnerOrder = ?");
...
/*  253 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*      */     try
/*      */     {
/*  290 */       pstmt = this.con.prepareStatement("INSERT INTO LNPFAndIImpQue VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
...
/*  383 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*      */     try
/*      */     {
/*  421 */       pstmt = this.con.prepareStatement("SELECT * FROM LNPFAndIImpQue WHERE  Questionid = ? AND InnerOrder = ?", 
/*  422 */         1003, 1007);
...
/*  442 */           tError.moduleName = "LNPFAndIImpQueDB";
...
/*  482 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  517 */     LNPFAndIImpQueSet aLNPFAndIImpQueSet = new LNPFAndIImpQueSet();
/*      */ 
/*  519 */     if (!this.mflag) {
...
/*  526 */       SQLString sqlObj = new SQLString("LNPFAndIImpQue");
/*  527 */       LNPFAndIImpQueSchema aSchema = getSchema();
...
/*      */       {
/*  535 */         i++;
/*  536 */         LNPFAndIImpQueSchema s1 = new LNPFAndIImpQueSchema();
/*  537 */         s1.setSchema(rs, i);
/*  538 */         aLNPFAndIImpQueSet.add(s1);
/*      */       }try {
...
/*  547 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  581 */     LNPFAndIImpQueSet aLNPFAndIImpQueSet = new LNPFAndIImpQueSet();
/*      */ 
/*  583 */     if (!this.mflag) {
...
/*      */       {
/*  595 */         i++;
/*  596 */         LNPFAndIImpQueSchema s1 = new LNPFAndIImpQueSchema();
/*  597 */         if (!s1.setSchema(rs, i))
/*      */         {
/*  600 */           CError tError = new CError();
/*  601 */           tError.moduleName = "LNPFAndIImpQueDB";
...
/*  615 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  649 */     LNPFAndIImpQueSet aLNPFAndIImpQueSet = new LNPFAndIImpQueSet();
/*      */ 
/*  651 */     if (!this.mflag) {
...
/*  658 */       SQLString sqlObj = new SQLString("LNPFAndIImpQue");
/*  659 */       LNPFAndIImpQueSchema aSchema = getSchema();
...
/*      */           break;
/*      */         }
/*  677 */         LNPFAndIImpQueSchema s1 = new LNPFAndIImpQueSchema();
/*  678 */         s1.setSchema(rs, i);
/*  679 */         aLNPFAndIImpQueSet.add(s1);
/*      */       }try {
...
/*  688 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  722 */     LNPFAndIImpQueSet aLNPFAndIImpQueSet = new LNPFAndIImpQueSet();
/*      */ 
/*  724 */     if (!this.mflag) {
...
/*      */           break;
/*      */         }
/*  746 */         LNPFAndIImpQueSchema s1 = new LNPFAndIImpQueSchema();
/*  747 */         if (!s1.setSchema(rs, i))
/*      */         {
/*  750 */           CError tError = new CError();
/*  751 */           tError.moduleName = "LNPFAndIImpQueDB";
...
/*  765 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  806 */       SQLString sqlObj = new SQLString("LNPFAndIImpQue");
/*  807 */       LNPFAndIImpQueSchema aSchema = getSchema();
/*  808 */       sqlObj.setSQL(2, aSchema);
/*  809 */       String sql = "update LNPFAndIImpQue " + sqlObj.getUpdPart() + " where " + strWherePart;
/*      */ 
/*  811 */       int operCount = stmt.executeUpdate(sql);
/*  812 */       if (operCount == 0)
/*      */       {
/*  815 */         CError tError = new CError();
/*  816 */         tError.moduleName = "LNPFAndIImpQueDB";
...
/*  836 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  873 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  893 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  940 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*  953 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*      */   {
/*  987 */     int tCount = 0;
/*  988 */     LNPFAndIImpQueSet tLNPFAndIImpQueSet = new LNPFAndIImpQueSet();
/*  989 */     LNPFAndIImpQueSchema tLNPFAndIImpQueSchema = null;
/*  990 */     if (this.mResultSet == null)
/*      */     {
/*  992 */       CError tError = new CError();
/*  993 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/*      */     {
/* 1001 */       tCount = 1;
/* 1002 */       tLNPFAndIImpQueSchema = new LNPFAndIImpQueSchema();
/* 1003 */       tLNPFAndIImpQueSchema.setSchema(this.mResultSet, 1);
/* 1004 */       tLNPFAndIImpQueSet.add(tLNPFAndIImpQueSchema);
/*      */ 
/* 1006 */       while (tCount++ < 5000)
/*      */       {
/* 1008 */         if (!this.mResultSet.next())
/*      */           continue;
/* 1010 */         tLNPFAndIImpQueSchema = new LNPFAndIImpQueSchema();
/* 1011 */         tLNPFAndIImpQueSchema.setSchema(this.mResultSet, 1);
/* 1012 */         tLNPFAndIImpQueSet.add(tLNPFAndIImpQueSchema);
/*      */       }
/*      */ 
/*      */     }
/*      */     catch (Exception ex)
...
/* 1019 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/* 1046 */       return null;
/*      */     }
/* 1048 */     return tLNPFAndIImpQueSet;
/*      */   }
/*      */ 
...
/* 1059 */         tError.moduleName = "LNPFAndIImpQueDB";
...
/* 1074 */       tError.moduleName = "LNPFAndIImpQueDB";
...
/* 1085 */         tError.moduleName = "LNPFAndIImpQueDB";
...
/* 1100 */       tError.moduleName = "LNPFAndIImpQueDB";
 ...
 * Qualified Name:     com.sinosoft.banklns.lis.db.LNPFAndIImpQueDB
